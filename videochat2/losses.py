...
class RRPO(object):
    ...
    def fetch_sign(self, targets):
        half_batch=targets.shape[0]//2
        pos_ids=targets[:half_batch].cpu().numpy()
        neg_ids=targets[half_batch:].cpu().numpy()

        pos_signs=[]
        neg_signs=[]
        for i in range(half_batch):
            _pos_signs, _neg_signs = get_diff_signs(seq1=pos_ids[i], 
                                                    seq2=neg_ids[i], 
                                                    min_match_size=1)
            pos_signs.append(_pos_signs)
            neg_signs.append(_neg_signs)

        signs = []
        signs.extend(pos_signs)
        signs.extend(neg_signs)

        signs = torch.tensor(signs, dtype=targets.dtype, device=targets.device)
        return signs
    ...
    def concatenated_forward(self, align_dict, ref_dict):
        ...
        # directly get the sign tensor from targets
        # signs=align_dict['signs'] ### this was old setup
        signs = self.fetch_sign(targets)

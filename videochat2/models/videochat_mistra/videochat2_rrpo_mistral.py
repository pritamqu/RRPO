...
from diff_lib import get_diff_signs

logger = logging.getLogger(__name__)

def process_masked_tokens_wrapper(tokens,
                                  return_tensors='pt'):
    # use first half as positive and second half as negative

    input_ids = tokens['input_ids']
    attention_mask = tokens['attention_mask']

    half_batch = input_ids.shape[0]//2
    pos_ids = input_ids[:half_batch]
    neg_ids = input_ids[half_batch:]

    pos_signs, neg_signs = get_diff_signs(pos_ids, neg_ids, min_match_size=3)

    signs = np.concatenate(pos_signs, neg_signs) # concat at dim 0

    if return_tensors=='pt':
        input_ids=torch.tensor([input_ids], dtype=torch.int64)
        attention_mask=torch.tensor([attention_mask], dtype=torch.int64)
        signs=torch.tensor([signs], dtype=torch.int64)

    return dict(input_ids=input_ids, attention_mask=attention_mask), signs
    

class VideoChat2_RRPO_Mistral(Blip2Base):
    """
    VideoChat2 model.
    """
    def __init__(self, config):
        ...
        logger.info(f'Loading Mistral from: {mistral_model_path}')
    ...
    def encode_img(self, image, instruction):
        ...
        with self.maybe_autocast():
            ...
            B, T, L, C = image_embeds.shape # TODO: we can add S3 Layer on this temporal axis meging it with spatial
    ...
    def default_forward(self, image, text_input, instruction, compute_ce_loss=True):
    ...
    def concatenated_forward(self, image, text_input, instruction):
        # img_embeds, use_image = self.encode_img(image, instruction)
        img_embeds1, use_image = self.encode_img(image, instruction) # torch.Size([4, 16, 3, 224, 224])
        img_embeds2, use_image = self.encode_img(image, instruction) # torch.Size([4, 16, 3, 224, 224])
        img_embeds = torch.cat([img_embeds1, img_embeds2], dim=0)
        batch_size, img_len, _ = img_embeds.shape

        pos_text_input = text_input['pos_conversation']
        neg_text_input = text_input['neg_conversation']

        text_input=[]
        text_input.extend(pos_text_input)
        text_input.extend(neg_text_input)
    ...
    def forward(self, image, text_input, instruction, mode='default'):

        if mode=='default': # original forward loop
            return self.default_forward(image, text_input, instruction)
            
        elif mode=='rrpo': 
            return self.concatenated_forward(image, text_input, instruction)

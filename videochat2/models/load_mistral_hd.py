...
def load_video(video_path, num_segments=8, return_msg=False, resolution=224, hd_num=12, padding=False):
...
def load_image(image_path, resolution=224, hd_num=12, padding=False):
...
def get_model_output_image(model, image_path, resolution, hd_num, padding,
                     question, max_new_tokens, device='cuda'):
    
    img = load_image(
        image_path, resolution, hd_num, padding=padding)
    ...
    with torch.no_grad():
        ...
        image_emb, _, _ = model.encode_img(img, question)
...
def get_model_output(model, vid_path, num_frame, resolution, hd_num, padding,
                     question, max_new_tokens, device='cuda',
        ):
    ...
    with torch.no_grad():
        ...
        image_emb, _, _ = model.encode_img(video, question)
...
def load_pretrained_model_mistral_hd(model_path):

    config_file_mistral = "configs/config_mistral_hd.json"
    ...
    state_dict = torch.load(f"{model_path}/videochat2_hd_mistral_7b_stage4.pth", "cpu")
    ...
    return model, resolution, num_frame, hd_num
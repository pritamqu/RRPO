...
class MetricLogger(object):
    ...
    def log_every_multi_loader(self, iterable, log_freq, header=None, interator_length=None):
        i = 0
        if not header:
            header = ''
        start_time = time.time()
        ...
        iter_time = SmoothedValue(fmt='{avg:.4f}')
        data_time = SmoothedValue(fmt='{avg:.4f}')
        space_fmt = ':' + str(len(str(interator_length))) + 'd'
        log_msg = [
            header,
            '[{0' + space_fmt + '}/{1}]',
            'eta: {eta}',
        ...
        for obj in iterable:
            ...
            yield obj
            iter_time.update(time.time() - end)
            if i % log_freq == 0 or i == interator_length - 1:
                eta_seconds = iter_time.global_avg * (interator_length - i)
                eta_string = str(datetime.timedelta(seconds=int(eta_seconds)))
                if torch.cuda.is_available():
                    logger.info(log_msg.format(
                        i, interator_length, eta=eta_string,
                        meters=str(self),
                        time=str(iter_time), data=str(data_time),
                        memory=torch.cuda.max_memory_allocated() / MB,
                        res_mem=torch.cuda.max_memory_reserved() / MB,
        ...
        total_time_str = str(datetime.timedelta(seconds=int(total_time)))
        logger.info('{} Total time: {} ({:.4f} s / it)'.format(
            header, total_time_str, total_time / interator_length))

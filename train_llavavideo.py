# Adopted from https://github.com/LLaVA-VL/LLaVA-NeXT/tree/main.
...
from llava.train.llava_trainer import LLaVATrainer, RRPOTrainer

from llava import conversation as conversation_lib
from llava.model import *
from llava.mm_utils import process_highres_image, process_anyres_image, process_highres_image_crop_split, tokenizer_image_token
from llava.utils import rank0_print, process_video_with_pyav, process_video_with_decord
from llava.losses import *
...
# taking a bit of conservative approach, as there could be some overlap in names between llm and vision encoder
...
def process_masked_tokens(input_ids, tokenizer):
    ...
    # UPDATED WITH add_special_tokens=False
    tag_start=[]
    ...
    while k<len(input_ids)-1:
        ...
        if [input_ids[k], input_ids[k+1]] in tag_start:
            ...
            k=k+1
            continue
            # raise ValueError('not handling all cases', [input_ids[k], input_ids[k+1]])
        ...
        # new_attention_mask.append(attention_mask[k])
...
def preprocess_masked(sources: Sequence[str], tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False) -> Dict:
    ...
    if conversation_lib.default_conversation.version == "qwen":
        ...
        ref_dict = preprocess_qwen(ref_sources, tokenizer, has_image=has_image)
        ref_input_ids = list(ref_dict['input_ids'][0].numpy())
        mask_dict = preprocess_qwen(sources, masked_toknizer, has_image=has_image)
        input_ids = mask_dict['input_ids']
        input_ids, signs = process_masked_tokens(input_ids, tokenizer=masked_toknizer)

        if input_ids==ref_input_ids:
            pass
        else:
            if len(input_ids)==len(ref_input_ids):
                ## check if there is a seq of 1s in diff_ids, the seq corresponding to signs should have same sign not a mixed
        ...
        ## return ref_input_ids and ref_labels
        return dict(**ref_dict, signs=signs)
    else:
        raise NotImplementedError()
...
class LazySupervisedDataset(Dataset):
    def __init__(self, tokenizer: transformers.PreTrainedTokenizer, data_args: DataArguments):
        ...
        self.max_frames = data_args.frames_upbound
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        pos_neg_dict['neg_image']=copy.deepcopy(pos_neg_dict['image'])
    ...
    def __pos_neg_getitem__(self, i)-> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0]: # noqa
            ...
            neg_sources = preprocess_multimodal(copy.deepcopy([e["conversations"] for e in neg_sources]), self.data_args)
            ...
            if os.path.exists(video_file):
                ...
                try:
                    ...
                    if video_file.endswith(".gif"):
                        ...
                        if len(images_group) < len(frame_idx) and self.data_args.force_sample:
        ...
        if isinstance(i, int):
            data_dict = dict(input_ids=data_dict["input_ids"][0], labels=data_dict["labels"][0], signs=data_dict["signs"][0])
            neg_data_dict = dict(input_ids=neg_data_dict["input_ids"][0], labels=neg_data_dict["labels"][0], signs=neg_data_dict["signs"][0],)
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        return dict(**pos_batch,
                    ...
                    neg_modalities=neg_batch['modalities'],
                    )

    def __subcall__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        input_ids, labels, signs = tuple([instance[key] for instance in instances] for key in ("input_ids", "labels", "signs"))
        # input_ids, labels, ids = tuple([instance[key] for instance in instances] for key in ("input_ids", "labels", "id"))
        input_ids = [_input_ids[: self.tokenizer.model_max_length] for _input_ids in input_ids]
        labels = [_labels[: self.tokenizer.model_max_length] for _labels in labels]
        signs = [_signs[: self.tokenizer.model_max_length] for _signs in signs]

        if self.tokenizer.pad_token_id is None:
            # self.tokenizer.pad_token_id = self.tokenizer.eos_token_id  # FIXME: (old) this could only be triggered for llama3 model.
            self.tokenizer.pad_token_id = 0 # This gets the best result. Don't know why.
        input_ids = self.pad_sequence(input_ids, batch_first=True, padding_value=self.tokenizer.pad_token_id)
        labels = self.pad_sequence(labels, batch_first=True, padding_value=IGNORE_INDEX)
        signs = self.pad_sequence(signs, batch_first=True, padding_value=IGNORE_INDEX)

        batch = dict(input_ids=input_ids, 
                     labels=labels.long() if labels.dtype == torch.int32 else labels, 
                     signs=signs.long() if signs.dtype == torch.int32 else signs, 
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if model_args.use_pos_skipping is not None and model_args.pos_skipping_range is not None and cfg_pretrained is not None:
    ...
    if model_args.mm_spatial_pool_stride is not None and model_args.mm_spatial_pool_out_channels is not None and model_args.mm_spatial_pool_mode is not None and model_args.mm_resampler_type is not None and cfg_pretrained:
    ...
    if overwrite_config:
        ...
        # {'use_pos_skipping': False, 'pos_skipping_range': 4096, 'mm_spatial_pool_mode': 'bilinear'}
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        if model_args.mm_tunable_parts is None:  # traditional way of deciding which part to train
            ...
            if "mm_language_model_lora" in tunable_parts:
            ...
            elif "mm_language_model" in tunable_parts:
                for name, param in model.named_parameters():
        ...
        if ref_model is not None:
            ...
            ref_model.config.mm_newline_position = model_args.mm_newline_position
            ref_model.config.add_faster_video = model_args.add_faster_video
            ref_model.config.faster_token_stride = model_args.faster_token_stride
            ref_model.config.mm_spatial_pool_stride = model_args.mm_spatial_pool_stride 
    ...
    # tokenizer handling in this code base is a bit messy, 
    # creating a separate copy for our mask tag handling
    global masked_toknizer
    masked_toknizer=copy.deepcopy(tokenizer)
    special_tokens = {'additional_special_tokens': ['<MASK>', '</MASK>']}
    masked_toknizer.add_special_tokens(special_tokens)
    sanity_check_token_len(masked_toknizer)

    rank0_print("params with grad on...")

...
class CambrianMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal_signed(
        ...
        if signs is None:
            raise ValueError('if signs tensor is None call prepare_inputs_labels_for_multimodal')
        ...
        signs = [
            cur_signs[cur_attention_mask]
            for cur_signs, cur_attention_mask in zip(signs, attention_mask)
        ]
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            for i in range(len(image_token_indices) - 1):
                ...
                cur_signs_noim.append(
                    cur_signs[image_token_indices[i] + 1 : image_token_indices[i + 1]]
                )
            ...
            for i in range(num_images + 1):
                ...
                if i < num_images:
                    ...
                    cur_new_signs.append(
                        torch.full(
                            (cur_image_features.shape[0],),
                            IGNORE_INDEX,
                            device=cur_signs.device,
                            dtype=cur_signs.dtype,
                        )
                    )
        ...
        new_signs_padded = torch.full(
            (batch_size, max_len),
            IGNORE_INDEX,
            dtype=new_signs[0].dtype,
            device=new_signs[0].device,
        )
        ...
        for i, (cur_new_embed, cur_new_labels, cur_new_signs) in enumerate(
            zip(new_input_embeds, new_labels, new_signs)
        ):
        ...
        if _signs is None: # technically i don't need this
            new_signs = None
        else:

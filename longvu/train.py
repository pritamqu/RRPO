...
def train() -> None:
    ...
    model_name_or_path='cambrian_qwen'

    # pyre-fixme[16]: `DataClass` has no attribute `vision_tower`.
    if model_args.vision_tower_aux_list is not None:
        if "cambrian" in model_args.input_model_filename.lower() or "cambrian" in model_name_or_path.lower():
            if "qwen" in model_args.input_model_filename.lower() or "cambrian" in model_name_or_path.lower():

...
def process_gifs(video_file, data_args):
    gif = imageio.get_reader(video_file)
    total_frame_num = len(gif)
    video_time = gif.get_meta_data()['duration'] / 1000  # Convert ms to seconds
    avg_fps = total_frame_num/video_time
    frame_idx = [i for i in range(total_frame_num)]
    interval = len(frame_idx) / float(data_args.frames_upbound)
    frame_idx = [int(interval * i) for i in range(data_args.frames_upbound)]
    ...
    if len(images_group) < len(frame_idx) and data_args.force_sample:

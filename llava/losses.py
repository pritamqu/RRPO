...
class RRPO(object):
    ...
    def __call__(self, align_dict, ref_dict, rank, world_size):
        ...
        metrics[f'kl_{train_test}/chosen'] = all_device_chosen_position_kl.cpu().float().numpy().tolist()
        metrics[f'kl_{train_test}/rejected'] = all_device_rejected_position_kl.cpu().float().numpy().tolist()
        metrics[f'kl_{train_test}/margin'] = (all_device_chosen_position_kl - all_device_rejected_position_kl).cpu().float().numpy().tolist()

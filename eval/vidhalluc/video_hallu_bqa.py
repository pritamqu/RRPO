...
if __name__ == "__main__":
    ...
    videos = '/datasets/video_llm/video_eval/VidHalluc/ach_binaryqa.json'

    with open(videos, 'r') as f:
        videos_dict = json.load(f)

    results = {}
    for index, question_list in tqdm(videos_dict.items()):
        ...
        for question in question_list:
            ...
            for clip in question['a']:
                prompt = "{desc} Only answer with a single word 'Yes' or 'No'.".format(desc=question['q'])
                video_path = video_folder + clip + '.mp4'
                temp_inner['a'][clip] = run_inference(dict(question=prompt, video_path=video_path),
                                                      tokenizer, model, image_processor)

...
def print_result(eval_results):
    with open("eval/tempcompass/meta_info.json", 'r') as f:
    ...
    return dict(result_asp=result_asp, result_fasp=result_fasp, match_rate=match_rate)
...
def get_llm_output(client, gpt_model, prompt, sys_prompt, max_tokens=128):
...
def get_gen_question(client, gpt_model, prompt, maxtry=10, sys_prompt=None, qtype='multi-choice', max_tokens=1000):
    llm_output, extracted_questions = None, None
    while True:
        try:
            llm_output = get_llm_output(client, gpt_model, prompt, sys_prompt, max_tokens)
...
def get_eval_result(client, gpt_model, prompt, maxtry=10, sys_prompt=None):
    llm_output = None
    while True:
        try:
            llm_output = get_llm_output(client, gpt_model, prompt, sys_prompt)

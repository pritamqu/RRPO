...
class MVBench_dataset(torch.utils.data.Dataset):
...
def run_inference(args):
    ...
    dataset = MVBench_dataset(args.data_dir, data_list, num_segments=num_frame, resolution=resolution)    
    if args.base_model_name in ["longvu_qwen_7b", "llavavideo_qwen_7b"]:
    ...
    with open(os.path.join(args.output_dir, f'mvbench.json'), "w") as f:
        json.dump({
            "acc_dict": acc_dict,
            "res_list": res_list
        }, f)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_dir', default="/datasets/video_llm/video_eval/MVBench")

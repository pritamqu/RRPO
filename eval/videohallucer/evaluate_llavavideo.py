...
import torch
from eval.videohallucer.evaluation_utils import cal_score, setup_seed
from llava.load_llavavideo import load_model, get_model_output_from_loaded_video, load_video, load_lora_weights
...
def main():
    ...
    parser.add_argument("--base_model_name", type=str, default="llavavideo_qwen_7b")
    ...
    parser.add_argument("--max_num_frames", type=int, required=False, default=64)
    ...
    for idx, batch in enumerate(tqdm(loader)):
        ...
        for key in basic_dict:
            basic_dict[key]=basic_dict[key][0]
        for key in halluc_dict:
            halluc_dict[key]=halluc_dict[key][0]        
        for key in common_dict:
            common_dict[key]=common_dict[key][0]        
        
        basic_video=basic_video[0]
        halluc_video=halluc_video[0]

        # channels are merged with time dim -- fix it
        # print(basic_video.shape, halluc_video.shape)

        # basic
        basic_pred=get_model_output_from_loaded_video(model, 
                                                      tokenizer,
                                                      image_processor,
                                                      basic_video, 
                                                      basic_dict['basic_question'], 
        ...
        # halluc
        halluc_pred=get_model_output_from_loaded_video(model, 
                                                       tokenizer,
                                                       image_processor,
                                                       halluc_video, 
                                                       halluc_dict['halluc_question'], 
                                                       args.model_max_length, 
                                                       temperature=0.0)
